...
function classify(v: number): RiskClass {
  if (v <= 25) return 'SAFE'
  if (v <= 50) return 'MONITOR'
  if (v <= 75) return 'ALERT'
  return 'CRITICAL'
}
...
export default function IRMSGauge({ value, riskClass }: Props) {
  ...
  const frac = v / 100
  const color = RISK_COLORS[classify(v)]
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        letterSpacing: 2, color: 'var(--ir-text-muted)',
